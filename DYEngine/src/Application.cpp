...
namespace DYE
{
	Application::Application(WindowProperties mainWindowProperties, int fixedFramePerSecond)
	{
		...
		DYE_LOG("GL Version: %d.%d, profile - %s", major, minor,
		...
		RenderPipelineManager::SetActiveRenderPipeline(std::make_shared<RenderPipeline2D>());
		// Use 2D RenderPipeline by default
